...
When a deployment to Active Directory is performed from a Concourse pipeline, we support the following payload types:
...
* `active-directory-deployment-to-servicenow.v1` forwards the payload from Concourse into 
...
* `terraform-deployment-from-concourse.v1` is supported on ingress and validates the event
  payload generated by the Concourse pipeline.
* `terraform-deployment-to-swift.v1` forwards the payload from Concourse into OpenStack
  Swift for archival purposes.
* `iterraform-deployment-to-servicenow.v1` forwards the payload from Concourse into the
  Change Management area of our ServiceNow instance.
...
| ----- | -------------------------------------------------------- | --------- | ----------- |
| `match_summary` | no | Regex-string | If present, the rule only applies to changes whose summary matches this regular expression. A leading `^` and trailing `$` anchor is added automatically, thus the regex has to match the whole summary. |
...
In order to get going in case of necessary changes to the process, these are the most important points to know about the ServiceNow (SNOW) API:

* Tenso does not interact with the SNOW api directly, but with a middleware API provided by CPI team which mostly routes requests through 1:1
  * this API uses certificate auth and maintainers get notified at `DL_67B5FEDE4D699DF1B5294E27@global.corp.sap` for cert renewal by CPI team
* Tenso's API interactions retroactively generate changes (`CHG` in SNOW) under pre-approved changes:
...
* `Change Model`s require fields/consistency of fields
  ...
  * e.g. `Assigned To` must be a user ID of the `Assignment Group` recorded in the template and fails silently/ causes `CHG`s to be stuck in open 
    * this check cannot be circumvented by design of SNOW
    ...
    * workaround: technical users as `Assigned To` are exempted from the validation
* if you want to find `CHG`s in SNOW, you could search by `Model`, `Assigned To` (=technical user), `Requested By` (=overall responsible for this type of change), `Implementation Contact` (=person triggering the change) or `Responsible Manager` (=manager of the overall responsible for this type of change)
